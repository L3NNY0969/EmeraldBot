module.exports.run = async (bot, msg, args) => {
    if(msg.channel.permissionsFor(bot.user).has("EMBED_LINKS") && msg.channel.permissionsFor(bot.user).has("MANAGE_MESSAGES")) {
        ...
        if(isNaN(toDelete)) {
            ...
            if(!toDelete || toDelete < 2 || toDelete > 100) return msg.channel.send(bot.embed({
                description: "Please provide a number between 2 and 100."
            }));
            msg.channel.bulkDelete(toDelete).then(() => {
                msg.channel.send(bot.embed({title: `Purged ${toDelete} messages!`})).then(m => m.delete(1500));
            });
        }
    } else {
