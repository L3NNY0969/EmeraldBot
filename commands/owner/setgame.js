module.exports.run = async (bot, msg, args) => {
    let match = /(p|play|play|l|listen|listening|w|watch|watching|s|stream|streaming|c|clear)/i.exec(args[0]);
    if(match) {
        switch(match[1]) {
            case "p":
            case "play":
            case "playing": {
                let newGame = args.join(" ").slice(args[0].length+1);
                if(!newGame) return msg.channel.send(`Invalid arguments passed. \`e.setgame [type:${match[1]}] [GAME]\``);
                else {
